...
def create_llm_client(config: Dict[str, Any]) -> Optional[Any]:
    ...
    if provider == "openai" or provider == "vllm":
        ...
        if api_key:
            logger.info("Using OpenAI API for safety checks")
            return OpenAI(api_key=api_key, base_url=base_url)
        logger.warning("OPENAI_API_KEY not found, trying alternative providers")
    
    # Fallback to Groq
    if provider == "groq":
        api_key = os.getenv("GROQ_API_KEY")
        if api_key:
            logger.info("Using Groq API for safety checks")
            return Groq(api_key=api_key)
        logger.warning("GROQ_API_KEY not found, safety checks will use fallback methods")
        return None
    
    # If OpenAI provider but no key, try Groq as fallback
    api_key = os.getenv("GROQ_API_KEY")
    if api_key:
        logger.info("Using Groq API as fallback for safety checks")
        return Groq(api_key=api_key)
    
    logger.warning("No API keys found, safety checks will use pattern-based fallback methods only")
    return None
...
{{
    ...
    try:
        # Use the client regardless of provider type (works for both OpenAI and Groq)
...
{{
    ...
    try:
        # Use the client regardless of provider type
        response = client.chat.completions.create(
            model=model_name,
            messages=[
